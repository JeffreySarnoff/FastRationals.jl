...
for (Q1, Q2) in ((:FastRational, :FastRational), (:FastRational, :Rational), (:Rational, :Rational))
  @eval begin
    function compactify1(midpoint::$Q1{T}, radius::$Q2{T}) where {T<:Integer}
        mid, rad = float(midpoint), float(abs(radius))
        lo, hi = mid-rad, mid+rad
        lo, hi = compact_rational_constraints1(mid, rad, lo, hi)
...
function compact_rational1(lo::T, hi::T) where {T<:Real}    
...
function compact_rational_constraints1(mid::T, rad::T, lo::T, hi::T) where {T<:Real}
